...
3. **Exécute** (`execute_plan`) en **ordre topologique** (`Plan::topo_order`, Kahn) : chaque tâche
   reçoit en contexte (borné) les sorties de ses dépendances, est lancée via `run_subagent`
   (mutualisé), puis son résultat est **tracé en mémoire** (`memory::append`, hand-off). Émet
   `TaskStarted`/`TaskDone`.
...
sequenceDiagram
    ...
    loop tâches en ordre topologique
        O-->>U: TaskStarted
        O->>A: run_subagent(objectif + contexte des dépendances)
